...
export class NGOMatchingAgent {
  ...
  static async findBestMatches(donation, maxMatches = 5) {
    ...
    const scoredNGOs = ngos.map(ngo => {
      ...
      // Could be enhanced with NGO capacity data
      const capacityScore = 15;

      // Availability score (0-20 points) - could check NGO schedule
      ...
      const totalScore = distanceScore + capacityScore + availabilityScore + urgencyAlignment;
  ...
  static async processNewDonation(donation) {
    try {
      ...
      if (matches.length === 0) {
        return {
          success: false,
          message: 'No NGOs with location data found',
          matches: [],
          notifications: []
        };
      }
   ...
   */
  static async enhanceWithAI(donation, ngos) {
...
- Location: ${donation.location} (${donation.latitude}, ${donation.longitude})
- Expires: ${donation.expires}
- Pickup Window: ${donation.pickupStart} to ${donation.pickupEnd}
- Notes: ${donation.notes || 'None'}

NGOs:
${ngos.map((ngo, i) => `${i + 1}. ${ngo.organizationName || ngo.name} - ${ngo.address} (${ngo.latitude}, ${ngo.longitude})`).join('\n')}
